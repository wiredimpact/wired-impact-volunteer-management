...
?>
	...
	if ( current_user_can( 'manage_options' ) ) :
		...
		$wi_form->form_table_start( 'email' );
			...
			$wi_form->textinput( 'admin_email_address', __( 'Admin Email Address', 'wired-impact-volunteer-management' ), array( 'description' => __( 'The person to notify when volunteers sign up. The contact for the volunteer opportunity will also be notified. If this field is blank then only the contact will be notified.', 'wired-impact-volunteer-management' ) ), null, 'send_signup_email_to_admins' );
			$wi_form->textinput( 'admin_signup_email_subject', __( 'Admin Signup Email Subject', 'wired-impact-volunteer-management' ), array( 'description' => __( 'The subject of the email to the admin after someone signs up to volunteer.', 'wired-impact-volunteer-management' ) ), null, 'send_signup_email_to_admins' );
			$wi_form->wysiwyg_editor( 'admin_signup_email', __( 'Admin Signup Email', 'wired-impact-volunteer-management' ), array( 'description' => __( 'The email to the admin after someone signs up to volunteer. You can use the variables {volunteer_first_name}, {volunteer_last_name}, {volunteer_phone}, {volunteer_email}, {opportunity_name}, {opportunity_date_time}, {opportunity_location}, {contact_name}, {contact_phone} and {contact_email} which will be replaced when the email is sent.', 'wired-impact-volunteer-management' ) ), 'send_signup_email_to_admins' );
			...
			$wi_form->textinput( 'days_prior_reminder', __( 'Number of Days Prior to Opportunity to Send Reminder', 'wired-impact-volunteer-management' ), array( 'description' => __( 'The number of days prior to a one-time volunteer opportunity to send a reminder. Ex: 4', 'wired-impact-volunteer-management' ) ), null, 'send_reminder_email_to_volunteers' );
			...
			$wi_form->wysiwyg_editor( 'volunteer_reminder_email', __( 'Volunteer Reminder Email', 'wired-impact-volunteer-management' ), array( 'description' => __( 'The reminder email to volunteers before their opportunity arrives. This is sent to the admins with the volunteers BCC\'ed. That way you know when the email has gone out. You can use the variables {opportunity_name}, {opportunity_date_time}, {opportunity_location}, {contact_name}, {contact_phone} and {contact_email} which will be replaced when the email is sent. Since only one email is sent do not use any of the volunteer specific variables.', 'wired-impact-volunteer-management' ) ), 'send_reminder_email_to_volunteers' );
