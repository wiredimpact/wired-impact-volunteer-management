...
<li>
 ...
 <?php
   ...
   the_title( sprintf( '<a href="%s" rel="bookmark">', esc_url( get_permalink() ) ), '</a>' ) . _e('</br>');
   ...
   ( $display_opp_when && ( $opp->opp_meta['one_time_opp'] || strlen( $opp->opp_meta['flexible_frequency'] ) > 0 ) ) ? $opp->display_meta( ( $opp->opp_meta['one_time_opp'] == 1 ) ? $opp->format_opp_times() : $opp->opp_meta['flexible_frequency'], __( 'When:', 'wired-impact-volunteer-management' ) ) . _e('</br>') : null;
   ...
   ( $display_opp_where && strlen( $opp->format_address() ) > 0 ) ? $opp->display_meta( $opp->format_address(),           __( 'Where:', 'wired-impact-volunteer-management' ) ) . _e('</br>') : null;
